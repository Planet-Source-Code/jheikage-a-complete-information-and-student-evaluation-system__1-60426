...
Begin VB.UserControl chameleonButton 
   ...
   EndProperty
   MouseIcon       =   "Xbutton.ctx":0000
   MousePointer    =   99  'Custom
   PropertyPages   =   "Xbutton.ctx":0152
   ScaleHeight     =   57
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   203
   ToolboxBitmap   =   "Xbutton.ctx":0187
...
Private Fons As Font
'Additions
Private iBold As Boolean, iIta As Boolean, iStr As Boolean, iName As String, iU As Boolean
Private SIX As Integer
...
End Property
Public Property Let BoldFont(ByVal NB As Boolean)
    iBold = NB
End Property
Public Property Get BoldFont() As Boolean
    BoldFont = iBold
    UserControl.FontBold = iBold
    Call CalcTextRects
    Call Redraw(0, True)
...
iName = newValue
If iName = "" Then iName = "MS Sans Serif"
UserControl.FontName = iName
Call CalcTextRects
Call Redraw(0, True)
PropertyChanged "INAME"
End Property
...
With PropBag
    'FontItalic = .ReadProperty("ITA", iIta)
    'BoldFont = .ReadProperty("BOLD", iBold)
    'FontSize = .ReadProperty("SIZE", SIX)
    FontUnderline = .ReadProperty("UND", iU)
    FontName = .ReadProperty("INAME", iName)
...
'PropertyChanged "INAME"
'PropertyChanged "SIZE"
'If UserControl.MousePointer = UserControl.MousePointer Then GoTo IRI
'UserControl.MousePointer = .ReadProperty("MPTR", UserControl.MousePointer)
IRI:
    If UserControl.Font = Font Then
    GoTo qq
    End If
    Set UserControl.Font = .ReadProperty("FONT", Fons)
qq:
    
    If UserControl.MouseIcon = UserControl.MouseIcon Then GoTo xx
    Set UserControl.MouseIcon = .ReadProperty("MICON", UserControl.MouseIcon)
xx:
    If picNormal Is Nothing Then GoTo x
    Set picNormal = .ReadProperty("PICN", Nothing)
x:
    If picHover Is Nothing Then GoTo Y
...
With PropBag
    Call .WriteProperty("BOLD", BoldFont)
    Call .WriteProperty("ITA", FontItalic)
    Call .WriteProperty("INAME", FontName)
    Call .WriteProperty("SIZE", FontSize)
    Call .WriteProperty("UND", FontUnderline)
...
'#@#@#@#@#@# BUTTON NORMAL STATE #@#@#@#@#@#
        Select Case MyButtonType
            ...
            Case 14 'KDE 2
                ...
                End If
                'If Ambient.DisplayAsDefault Then isShown = False: prevBold = Me.BoldFont
                Call DrawCaption(Abs(isOver))
                If Ambient.DisplayAsDefault Then Me.BoldFont = prevBold: isShown = True
